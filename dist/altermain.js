...
const validatePhoneNumberFunc = (input)=>{
    return ()=>{
        const toAllowedPhoneNumber = (pN)=>{
            const nondash = pN.replace(/[-|\s]/g, '');
            if (pN === '') {
                return;
            }
            if (isNaN(Number(nondash)) || nondash.includes('.')) {
                input.value = input.prevVal || '';
                throw new Error("invalid input!");
            }
            return nondash;
        };
        let num = input.value;
        num = toAllowedPhoneNumber(num) || '';
        const phoneFormat = /([0-9]{3})([0-9]+)([0-9]{4})/;
        input.value = num.replace(phoneFormat, '$1-$2-$3');
        input.prevVal = input.value;
    };
};
class InputPhoneNumberAlt {
    _input;
    _submit;
    _reset;
    _submitFunc;
